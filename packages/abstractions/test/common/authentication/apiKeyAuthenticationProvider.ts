...
import { ApiKeyAuthenticationProvider, ApiKeyLocation } from "../../../src/authentication";
...
describe("ApiKeyAuthenticationProvider", () => {
	it("Throws on invalid initialization", () => {
		assert.throws(() => new ApiKeyAuthenticationProvider("", "param", ApiKeyLocation.QueryParameter));
		assert.throws(() => new ApiKeyAuthenticationProvider("param", "", ApiKeyLocation.QueryParameter));
		assert.throws(() => new ApiKeyAuthenticationProvider("param", "key", 2 as unknown as ApiKeyLocation));
	});
	it("Adds in query parameters", async () => {
		const provider = new ApiKeyAuthenticationProvider("key", "param", ApiKeyLocation.QueryParameter);
	...
	it("Adds in headers", async () => {
		const provider = new ApiKeyAuthenticationProvider("key", "param", ApiKeyLocation.Header);
		const request = new RequestInformation();
		request.urlTemplate = "https://localhost{?param1}";
		await provider.authenticateRequest(request);
