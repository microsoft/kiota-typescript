...
describe("Test authentication using SharePoint Framework", () => {
	it("AccessToken is returned correctly from getAuthorizationToken function", async () => {
		...
		const allowedHosts: Set<string> = new Set<string>(["myapi.azurewebsites.net"]);

		const mockTokenProvider = new MockAadTokenProvider(expectedToken);

		const accessTokenProvider = new AzureAdSpfxAccessTokenProvider(mockTokenProvider as any, appIdUri, allowedHosts);

		const accessToken: string = await accessTokenProvider.getAuthorizationToken("https://myapi.azurewebsites.net/endpoint");
	...
	it("AccessToken is empty when requested URL is not in allowedHosts", async () => {
		...
		const accessToken: string = await accessTokenProvider.getAuthorizationToken("https://notallowedhost.azurewebsites.net/endpoint");
	...
	it("AccessToken is appended correctly in header by AzureAdSpfxAuthenticationProvider", async () => {
		...
		const azureAdSpfxAuthProvider = new AzureAdSpfxAuthenticationProvider(mockTokenProvider as any, appIdUri, allowedHosts);
		...
		assert.equal(request.headers.tryGetValue("Authorization")![0], "Bearer " + expectedToken);
	});
});
