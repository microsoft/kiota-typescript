...
describe("Test authentication using @azure/identity", () => {
	...
	it("AccessToken is returned correctly from getAuthorizationToken function", async () => {
		...
		const accessTokenProvider = new AzureIdentityAccessTokenProvider(clientCredential, scopes, undefined, new Set<string>(["graph.microsoft.com"]));
		const access = await accessTokenProvider.getAuthorizationToken("https://graph.microsoft.com/v1.0");
	...
	it("AccessToken is appended correctly in header by AzureIdentityAuthenticationProvider", async () => {
		...
		assert.equal(request.headers.tryGetValue("Authorization")![0], "Bearer " + accessToken.token);
	});
	...
	it("adds the claims to the token context", async () => {
		...
		const moq = sinon.mock(clientCredential);
		moq.expects("getToken")
			.exactly(1)
			.callsFake((_, options) => {
				assert.equal(options.claims, '{"access_token":{"nbf":{"essential":true, "value":"1652813508"}}}');
