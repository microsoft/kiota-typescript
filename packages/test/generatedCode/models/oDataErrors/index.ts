...
export function deserializeIntoErrorDetails(errorDetails: Partial<ErrorDetails> | undefined = {}): Record<string, (node: ParseNode) => void> {
	return {
		code: (n) => {
			errorDetails.code = n.getStringValue();
		},
		message: (n) => {
			errorDetails.message = n.getStringValue();
		},
		target: (n) => {
			errorDetails.target = n.getStringValue();
		},
	};
}
/**
...
export function deserializeIntoMainError(mainError: Partial<MainError> | undefined = {}): Record<string, (node: ParseNode) => void> {
	return {
		code: (n) => {
			mainError.code = n.getStringValue();
		},
		details: (n) => {
			mainError.details = n.getCollectionOfObjectValues<ErrorDetails>(createErrorDetailsFromDiscriminatorValue);
		},
		innerError: (n) => {
			mainError.innerError = n.getObjectValue<InnerError>(createInnerErrorFromDiscriminatorValue);
		},
		message: (n) => {
			mainError.message = n.getStringValue();
		},
		target: (n) => {
			mainError.target = n.getStringValue();
		},
	};
}
/**
...
export function deserializeIntoODataError(oDataError: Partial<ODataError> | undefined = {}): Record<string, (node: ParseNode) => void> {
	return {
		error: (n) => {
			oDataError.errorEscaped = n.getObjectValue<MainError>(createMainErrorFromDiscriminatorValue);
			oDataError.message = oDataError.errorEscaped?.message ?? "";
		},
	};
}
